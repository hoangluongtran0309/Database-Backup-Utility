...
@Component("postgresqlConnect")
public class PostgreSQLConnect implements ConnectService {

    private static final Logger logger = LoggerFactory.getLogger(PostgreSQLConnect.class);

    @Override
    public void connect(ConnectionParams connectionParams) throws DatabaseConnectionException {
        String uri = String.format("jdbc:postgresql://%s:%d/%s",
        ...
        try (Connection connection = DriverManager.getConnection(
            ...
            if (connection.isValid(30)) {
                logger.info("Successfully connected to PostgreSQL at {}:{}/{}",
                        connectionParams.getHost(),
                        connectionParams.getPort(),
                        connectionParams.getDatabaseName());
            } else {
                String error = String.format("Invalid PostgreSQL connection at %s:%d/%s",
                        connectionParams.getHost(),
                        connectionParams.getPort(),
                        connectionParams.getDatabaseName());
                logger.error(error);
            ...
            String error = String.format("Failed to connect to PostgreSQL at %s:%d/%s - %s",
                    connectionParams.getHost(),
                    connectionParams.getPort(),
                    connectionParams.getDatabaseName(),
                    e.getMessage());
